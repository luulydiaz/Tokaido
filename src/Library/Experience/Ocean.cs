...
namespace Tokaido
{
    /*
            ...
            Ocean usa el patron Observer y por lo tanto es un observador y el metodo PositionChange
            recibe actualizaciones cuando cambia de posicion el viajero.
            PositionChange tambien es Polimorfica y por lo tanto Ocean la define segun sus necesidades
            Usa Expert para asignar los puntos y remover a los viajeros, ya que conoce a los viajeros que se ingresaron.
    */
    public class Ocean: AbstractExperience
    {
        ...
        public Ocean(string name, int limit, int position): base(name,limit,position)
        {
        }
        ...
        public void GivePoints()
        {
            foreach(AbstractTraveler traveler in this.Travelers)
            {
                ...
                {
                    traveler.Score = (traveler.Score + (traveler.VisitedOceans*2) + 1);
                    this.TravelersPoint.Add(traveler);
                    traveler.VisitedOceans += 1;
                }
            }
        }
